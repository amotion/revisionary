...
function onLoaded() {
    ...
    var get_settings = "$._ext_get_settings.run()";

	evalScript(get_settings, function (response) {

        response = response.split(',');

		document.getElementById('db-id').value = response[0];
		document.getElementById('db-token').value = response[1];
		document.getElementById('db-token-type').value = response[2];
...
function updateThemeWithAppSkinInfo(appSkinInfo) {
    ...
    $('body').removeClass().addClass('theme_'+toHex(panelBackgroundColor).substring(1));
